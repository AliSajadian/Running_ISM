...
export default {
  ...
  methods: {
    ...
    reload() {
      ...
      const currentFilter = this.filters.find(f => f.lable === this.forms.shipments.filter) || this.filters[0];
  
      // Reload the shipment data with the current filter
      this.report_shipment(currentFilter.value);
    ...
    countdown() {
      if (this.totalSeconds > 0) {
        ...
        const currentFilter = this.filters.find(f => f.lable === this.forms.shipments.filter);
        const filterValue = currentFilter ? currentFilter.value : 'year';
        this.report_shipment(filterValue);
