...
export default {
  ...
  methods: {
    ...
    reload() {
      ...
      const currentFilter = this.filters.find(f => f.lable === this.forms.products.filter) || this.filters[0];
  
      // Reload the products data with the current filter
      this.report_Products(currentFilter.value);
    ...
    countdown() {
      if (this.totalSeconds > 0) {
        ...
        const currentFilter = this.filters.find(f => f.lable === this.forms.products.filter);
        const filterValue = currentFilter ? currentFilter.value : 'year';
        this.report_Products(filterValue);
