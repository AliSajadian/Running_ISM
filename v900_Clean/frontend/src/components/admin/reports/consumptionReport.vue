...
export default {
  ...
  methods: {
    ...
    reload() {
      ...
      const currentFilter = this.filters.find(f => f.lable === this.forms.consumption.filter) || this.filters[0];
  
      // Reload the consumption data with the current filter
      this.report_Consumption(currentFilter.value);
    ...
    countdown() {
      if (this.totalSeconds > 0) {
        ...
        const currentFilter = this.filters.find(f => f.lable === this.forms.consumption.filter);
        const filterValue = currentFilter ? currentFilter.value : 'year';
        this.report_Consumption(filterValue);
