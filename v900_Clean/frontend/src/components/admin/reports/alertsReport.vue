...
export default {
  ...
  methods: {
    ...
    reload() {
      ...
      const currentFilter = this.filters.find(f => f.lable === this.forms.alerts.filter) || this.filters[0];
  
      // Reload the alerts data with the current filter
      this.report_alerts(currentFilter.value);
    ...
    countdown() {
      if (this.totalSeconds > 0) {
        ...
        const currentFilter = this.filters.find(f => f.lable === this.forms.alerts.filter);
        const filterValue = currentFilter ? currentFilter.value : 'year';
        this.report_alerts(filterValue);
