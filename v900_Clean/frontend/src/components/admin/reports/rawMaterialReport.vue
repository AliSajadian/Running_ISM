...
export default {
  ...
  methods: {
    ...
    reload() {
      ...
      const currentFilter = this.filters.find(f => f.lable === this.forms.rawMaterial.filter) || this.filters[0];
  
      // Reload the rawMaterial data with the current filter
      this.report_RawMaterial(currentFilter.value);
    ...
    countdown() {
      if (this.totalSeconds > 0) {
        ...
        const currentFilter = this.filters.find(f => f.lable === this.forms.rawMaterial.filter);
        const filterValue = currentFilter ? currentFilter.value : 'year';
        this.report_RawMaterial(filterValue);
